...
- Camera framing for the playfield should keep the desktop orthographic zoom stable across level lengths and use damped X/Z follow offsets. Phone-sized viewports automatically switch to a row-aligned camera with full horizontal tracking, a board width spanning `1.5` viewport widths, and direction-aware vertical framing that places upward movement on the lower-third line while opening more space below during downward movement.
